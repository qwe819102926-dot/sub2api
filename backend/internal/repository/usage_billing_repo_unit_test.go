...
const (
	...
	bonusBalanceSelectSQL       = `SELECT COALESCE\(bonus_balance, 0\) FROM users WHERE id = \$1 AND deleted_at IS NULL FOR UPDATE`
	bonusBalanceDeductSQL       = `UPDATE users SET bonus_balance = bonus_balance - \$1, updated_at = NOW\(\) WHERE id = \$2`
	currentBalanceSelectSQL     = `SELECT balance FROM users WHERE id = \$1 AND deleted_at IS NULL`
...
func TestDeductUsageBillingBalance_UsesSufficientBalanceGuard(t *testing.T) {
	...
	mock.ExpectQuery(bonusBalanceSelectSQL).
		WithArgs(int64(42)).
		WillReturnRows(sqlmock.NewRows([]string{"bonus_balance"}).AddRow(0))
	mock.ExpectQuery(conditionalBalanceDeductSQL).
		WithArgs(2.5, int64(42)).
		WillReturnRows(sqlmock.NewRows([]string{"balance"}).AddRow(7.5))
	mock.ExpectCommit()

	newBalance, sufficient, _, _, err := deductUsageBillingBalanceDetailed(ctx, tx, 42, 2.5)
	require.NoError(t, err)
...
func TestDeductUsageBillingBalance_RecordsOverdraftWhenGuardMisses(t *testing.T) {
	...
	newBalance, sufficient, _, _, err := deductUsageBillingBalanceDetailed(ctx, tx, 42, 10)
	require.NoError(t, err)
...
func TestDeductUsageBillingBalance_ReturnsUserNotFoundWhenNoUserUpdated(t *testing.T) {
	...
	_, _, _, _, err = deductUsageBillingBalanceDetailed(ctx, tx, 42, 10)
	require.ErrorIs(t, err, service.ErrUserNotFound)
	require.NoError(t, tx.Rollback())
	require.NoError(t, mock.ExpectationsWereMet())
}

func TestDeductUsageBillingBalance_UsesBonusBalanceBeforePrincipal(t *testing.T) {
	ctx := context.Background()
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	defer func() { _ = db.Close() }()
	...
	require.NoError(t, err)
	require.True(t, sufficient)
	require.InDelta(t, 10.0, newBalance, 0.000001)
	require.NoError(t, tx.Commit())
	require.NoError(t, mock.ExpectationsWereMet())
}
