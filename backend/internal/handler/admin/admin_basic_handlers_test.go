...
func setupAdminRouter() (*gin.Engine, *stubAdminService) {
	...
	router.POST("/api/v1/admin/users/:id/bonus-balance", userHandler.UpdateBonusBalance)
...
func TestUserHandlerEndpoints(t *testing.T) {
	...
	rec = httptest.NewRecorder()
	req = httptest.NewRequest(http.MethodPost, "/api/v1/admin/users/1/bonus-balance", bytes.NewBufferString(`{"balance":1,"operation":"add"}`))
	req.Header.Set("Content-Type", "application/json")
	router.ServeHTTP(rec, req)
	require.Equal(t, http.StatusOK, rec.Code)
