...
func validateProviderSnapshotMetadata(order *dbent.PaymentOrder, providerKey string, metadata map[string]string) error {
	...
	case payment.TypeJianPay:
		if expected := strings.TrimSpace(snapshot.MerchantID); expected != "" {
			actual := strings.TrimSpace(metadata["client_no"])
			if actual == "" {
				return fmt.Errorf("jianpay clientNo missing")
			}
			if !strings.EqualFold(expected, actual) {
				return fmt.Errorf("jianpay clientNo mismatch: expected %s, got %s", expected, actual)
			}
		}
